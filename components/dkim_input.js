...
import { addDKIM, getDKIMStatus } from "@/lib/helpers";
import IsRunning from "./../lib/isRunning";

function handleAddDKIM(ref) {
...
function handleShowStatus(ref, setStatus, setStatusLoading) {
  ...
  getDKIMStatus(data).then((res) => {
    setStatus(res);
    setStatusLoading(false);
  });
}

export default function DKIMDomainCard({ setStatus }) {
  const ref = useRef();
  ...
  return (
    <Card className="flex flex-col h-[calc(100vh-3rem)]">
      <CardHeader>
        ...
        <CardDescription>
          Provide list of domains to generate dkim keys
        </CardDescription>
      </CardHeader>
      ...
      <CardFooter className="flex justify-between">
        ...
        <Button
          ...
          {isRunning ? "Running" : "Generate DKIM"}
        </Button>
      </CardFooter>
    </Card>
  );
